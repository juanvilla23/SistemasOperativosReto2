...
int main () {
    ...
    while (fgets (my_message.message_text.buffer, 198, stdin)) {
        // remove newline from string
        int length = strlen (my_message.message_text.buffer);
        if (my_message.message_text.buffer [length - 1] == '\n')
           my_message.message_text.buffer [length - 1] = '\0';
        ...
        printf ("Message received from server: %s\n\n", return_message.message_text.buffer);  
